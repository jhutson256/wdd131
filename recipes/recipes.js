...
function search() {
    let recipeQuery = document.querySelector('#search-box').value;
...
function recipeTemplate(recipe) {
    return `
        <div class="recipe">
            <img id="food-img" src=${recipe.image} alt=${recipe.name}>
            <div class="recipe-info">
                ...
                <div id="recipe-description">
                    <h2>${recipe.name}</h2>
                    <p>${recipeRating(recipe.rating)}</p>
...
function tagTemplate(tags) {
    return tags.map((tag)=> `<button id="recipe-type">${tag}</button>`).join(' ');
}
